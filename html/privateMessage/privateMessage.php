...
foreach ($conversation['privateMessages'] as $message) {
    ...
    try {
        if ($_SESSION["userId"] == $message['to_user_id']){
            $response = $client->get('http://api/users/' . $message["from_user_id"]);
            $user = json_decode($response->getBody(), true)['users'];

            $response = $client->get('http://api/getPpById/' . $message["from_user_id"]);
        } else {
            $response = $client->get('http://api/users/' . $message["to_user_id"]);
            $user = json_decode($response->getBody(), true)['users'];

            $response = $client->get('http://api/getPpById/' . $message["to_user_id"]);
        }
